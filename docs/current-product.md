...
- My Pokemon now reads the same anonymous-session-backed server collection state as daily and uses the same reduced catalog payload shape.
- Team builder and My Teams now store team data per anonymous session in PostgreSQL, and the team builder route now uses a reduced catalog payload instead of the previous full catalog route payload.
