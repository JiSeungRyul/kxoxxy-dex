...
## Footprint Icon Dark Mode Invert (Added: 2026-09-22)
- Symptom: the 발자국(footprint) icon on the Pokemon detail page (`DetailImageItem`, `pokemon-detail-page.tsx`) stayed solid black in dark mode instead of flipping to a visible light color.
- Root cause: `public/footprint.svg` uses `fill="currentColor"`, but it's rendered through Next.js `<Image>`, which emits a plain `<img src="/footprint.svg">`. An externally-referenced SVG loaded via `<img>` is rendered in its own isolated document context — `currentColor` there resolves against the SVG's own default color, not the page's CSS, so the app's theme/foreground color never reaches it.
- Also found while fixing this: the codebase's existing `dark:` Tailwind utility usages elsewhere (`pokedex-table.tsx` captured-row background, `daily-encounter.tsx`, `TYPE_BADGE_STYLES` in `constants.ts`) are wired to Tailwind's default `darkMode: 'media'` strategy (OS `prefers-color-scheme`), **not** this app's manual theme toggle, which instead flips a `.dark` class on `<html>` (see `app/layout.tsx` `themeScript` and `.dark` selector in `app/globals.css`). `tailwind.config.ts` never sets `darkMode: 'class'`. This means those existing `dark:`-prefixed classes only activate when the OS itself is in dark mode, independent of the in-app toggle — a real but separate, pre-existing inconsistency, out of scope for this fix and not touched here.
- Fix: used an arbitrary-variant selector tied directly to the actual `.dark` ancestor class instead of the (mismatched) `dark:` utility — `className="h-8 w-8 object-contain [.dark_&]:invert"` on the `<Image>` in `DetailImageItem`. This compiles to `.dark img[...] { filter: invert(100%) }`, which correctly tracks the app's real manual toggle regardless of OS theme. Verified the compiled CSS output directly (`grep` on the Turbopack build's `.css` chunk) to confirm the rule generated as `.dark .\[\.dark_\&\]\:invert{...filter:...invert...}`.
- If the broader `dark:`-utility-vs-manual-toggle mismatch elsewhere is worth fixing, the correct fix would be adding `darkMode: 'class'` to `tailwind.config.ts` — flagged here for a future task, not applied now (broader blast radius across multiple files, not requested).

## /pokedex Table Horizontal Scroll Fix — Main List (Added: 2026-09-22)
- Follow-up to the detail-page scroll fix below: user confirmed the actually-reported bug was on the main pokedex list (`/`), not the detail page — stat columns (특수방어 etc., shown at `sm:` breakpoint and up) got visually cut off at the right edge with no way to swipe/scroll to see the rest.
- Root cause: `features/pokedex/components/pokedex-table.tsx` wrapped the `<table>` in `<div className="px-5 py-5 sm:px-8">` with no scroll container, inside a `<section>` that has `overflow-hidden`. When the table's natural width exceeded the viewport, the outer `overflow-hidden` clipped the excess instead of allowing it to scroll — there was no `overflow-x-auto` anywhere in this component, so the content was unconditionally unreachable past the clip edge, not just hard to scroll.
- Verified before fixing: reproduced the same `overflow-hidden` (no scroll wrapper) + `overflow-x-auto` (dedicated scroll wrapper) markup pair in an isolated headless-Chromium touch-swipe test. The `overflow-hidden` version never moved (`scrollLeft` stayed `0` after a full swipe gesture); the `overflow-x-auto` version scrolled to its max immediately. This confirms the root cause precisely (not just a Safari-only quirk, as first assumed for the detail-page entry below).
- Fix: changed the table's inner wrapper from `<div className="px-5 py-5 sm:px-8">` to `<div className="overflow-x-auto overscroll-x-contain px-5 py-5 [-webkit-overflow-scrolling:touch] sm:px-8">`. The outer `<section>`'s `overflow-hidden` is kept (only clips at the rounded-card boundary now, not the table content) — same nested-scroll-container pattern already used in `pokemon-detail-page.tsx`.
- Verified: `npm run typecheck` passed; isolated touch-scroll simulation confirmed the fix (see above). Real-device confirmation still recommended after deploy.

## Mobile Horizontal Scroll Touch Fix — Pokemon Detail Page (Added: 2026-09-22)
- Symptom: on mobile, inside a Pokemon's detail page (reached from `/` → pokedex list → pokemon detail), the base stats row is wider than the screen and gets visually cut off around 특수방어(Sp. Def); swiping with a finger to reveal the remaining stats (스피드, 합계) did not move the content.
- Investigated: `features/pokedex/components/pokemon-detail-page.tsx` has 5 horizontally-scrollable rows (`overflow-x-auto` + `flex min-w-max`) — base stats, ability table, and 3 evolution-chain layouts. Reproduced the exact Tailwind class combination in an isolated headless-Chromium touch-drag test; the drag mechanics themselves scrolled correctly there, and no JS touch handlers or global `touch-action`/`overscroll-behavior` rules were found blocking it anywhere in the app. This points to the well-known Safari/iOS gap where a nested `overflow-x: auto` container without `-webkit-overflow-scrolling: touch` and `overscroll-behavior-x` can fail to respond to a normal flick/swipe (it can still work with the app compiled in a WSL container's Chromium engine but not necessarily match real-device iOS Safari behavior, which could not be verified on a physical device in this environment).
- Fix: added `overscroll-x-contain [-webkit-overflow-scrolling:touch]` to all 5 `overflow-x-auto` wrappers in `pokemon-detail-page.tsx` (base stats, ability table, evolution chain x3) — enables iOS momentum/touch scrolling for the container and stops the swipe gesture from chaining into the page's vertical scroll or the browser's edge-swipe-back gesture.
- Verified: `npm run typecheck` passed. Real-device mobile confirmation not done (no physical/remote iOS device available in this environment) — recommend confirming on an actual phone after deploy.
