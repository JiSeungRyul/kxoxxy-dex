...
- `26). 서버 기반 사용자 영속화 확장`
- `27). Favorites`
- `27-1). 즐겨찾기 DB 스키마 설계 및 마이그레이션 (favorite_pokemon 테이블)`
- `27-2). 즐겨찾기 상태 관리를 위한 서버 repository 로직 추가 (toggle/get)`
- `27-3). 익명 세션 쿠키 기반의 즐겨찾기 상태 API 구현 (/api/favorites/state)`
- `27-4). 포켓몬 상세 페이지([slug])에 즐겨찾기 토글 버튼 추가 및 연동`
- `27-5). 포켓몬 목록(메인 도감) UI에 즐겨찾기 표시 및 토글 버튼 추가`
- `27-6). 즐겨찾기 전용 모아보기 페이지 구현 (/favorites)`
- `27-7). 헤더 네비게이션에 즐겨찾기 메뉴 추가`
