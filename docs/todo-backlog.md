...
- `21-3-6-5-1-4). 지역 폼 중에서도 전설/환상 조류(가라르 프리져/썬더/파이어)와 복합 규칙 종(팔데아 켄타로스)은 각 전용 backlog 항목으로 유지`
- `21-3-6-5-2). 전설/환상 포켓몬 전용 폼 확장 검토 (기라티나/쉐이미/버드렉스 등)`
- `21-3-6-5-3). 다수 폼/복합 규칙 폼 확장 검토 (켄타로스 팔데아 breed, 지가르데 등)`
- `21-3-6-5-4). 일반 폼 라벨 한글화 및 selector 카피 보정 필요 대상 정리`
