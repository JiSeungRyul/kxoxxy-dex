...
### 2. 모바일 UI 대응 (구 2)
- [ ] `2-1). 모바일 UI 전반 점검 및 보완 — 햄버거 메뉴, 컨트롤 레이아웃, 배지 위치, 능력치 테이블 스크롤 등`
- [ ] `2-2). 상단 계정 영역에 Google 이름 대신 닉네임 표시 (닉네임 미설정 시 폴백 처리)`
- [ ] `2-3). 포켓덱스 모바일 환경 좌우 스크롤 제거 — 두 줄 허용 방식으로 레이아웃 조정`
