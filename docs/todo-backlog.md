...
- `35-2-1). Soft Delete 계정 비활성 사용자 세션 차단 및 보호 라우트 처리`
- `35-2-2). 계정 삭제 요청 route / UI 진입점 추가`
- `35-2-3). 계정 삭제 후 grace period 동안 복구 가능 여부와 재활성화 흐름 정리`
- `35-2-4). grace period 만료 후 favorites / daily / teams 실제 purge 정책 및 작업 방식 정리`
- `35-2-5). 사용자 데이터 초기화 범위(즐겨찾기 / 내 포켓몬 / 팀)와 실행 순서 정의`
