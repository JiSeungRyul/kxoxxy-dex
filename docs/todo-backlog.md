...
- `x16). 팀 생성 액션 문구 개선`
- `17). 팀빌더 레벨 설정 가능하게 (레벨 변경 시 실전 능력치 재계산)`
...
- `18). 특성 드롭다운 도입`
- `19). 아이템 검색 및 드롭다운 도입`
- `20). 배우기 가능한 기술 드롭다운 가능 여부 검토`

이유: 가치가 있지만 데이터 구조와 UI 복잡도가 올라갑니다. 지금은 “검토” 성격이 강해서 바로 앞순위는 아닙니다.

## 6. 세션/데이터 모델 강화
- `21). 브라우저 생성 익명 세션을 더 강한 서버 관리 세션으로 교체`
- `22). anonymous-session에서 향후 user_id 기반 ownership으로 이동할 경로 정리`
- `23). migration 적용/서버 재시작 체크 보강`
...
- `25). 서버 기반 사용자 영속화 확장`
- `26). Favorites`
- `27). 하이브리드 이후 추가 DB 통합`
- `28). 계정 기반 ownership 전환`
- `29). 더 깊은 catalog 정규화 재검토`
