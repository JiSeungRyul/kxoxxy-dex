...
### 1. 즐겨찾기 접근성 개선 (구 47)
- [x] `1-1). 즐겨찾기를 /my 페이지 외 헤더/네비게이션 등 다른 경로에서도 접근 가능하도록 개선`

### 2. 닉네임 설정 플로우 (구 44)
- [x] `2-1). 최초 로그인 시 닉네임 설정 플로우 추가 (현재는 Google 계정 이름이 그대로 표시됨)`
- [x] `2-2). 닉네임 변경 UI를 /my 페이지에 추가`

### 3. CI/CD 자동 배포 파이프라인 (구 50)
- [ ] `3-1). GitHub Actions → 서버 pull/build/restart 자동화 파이프라인 도입`
