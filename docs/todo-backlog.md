...
- `18-1). 대전 모드에서 종 중복 경고를 저장 차단 규칙으로 올릴지 검토`
- `18-2). 대전 모드 아이템 중복 경고를 추후 아이템 id 비교 기반 검증으로 확장 검토 (현재는 텍스트 기준)`
- `18-3). 모드별 기본 레벨/설명 프리셋 확장 검토`
