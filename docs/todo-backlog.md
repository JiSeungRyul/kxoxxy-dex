...
### 44. 계정 프로필 설정
- [ ] `44-1). 최초 로그인 시 닉네임 설정 플로우 추가 (현재는 Google 계정 이름이 그대로 표시됨)`
- [ ] `44-2). 닉네임 변경 UI를 /my 페이지에 추가`

### 45. 개발용 로그인 제거
- [x] `45-1). 운영 환경에서 개발용 fallback 로그인 경로 완전 제거 (Google provider 모드에서만 동작하도록 강제)`

### 46. 모바일 UI 대응
- [ ] `46). 모바일 접속 시 레이아웃/UI 깨짐 전체 수정`
- [ ] `46-1). 수정 대상 리스트는 별도로 전달 예정`

### 47. 즐겨찾기 접근성 개선
- [ ] `47-1). 즐겨찾기를 /my 페이지 외 다른 경로(헤더, 네비게이션 등)에서도 접근 가능하도록 개선`

### 48. 포켓덱 이름 검색 깜빡임 개선
- [x] `48-1). 이름 검색 필터 입력 시 포켓몬 이름 텍스트가 계속 바뀌며 화면이 깜빡이는 문제 수정`

### 49. 내 팀 보기 아코디언 UI
- [x] `49-1). /my-teams 팀 목록을 팀별로 접고 펼치는 아코디언 구조로 변경 (기본값: 모두 접힘)`

### 50. 자동 배포 시스템
- [ ] `50-1). CI/CD 자동 배포 파이프라인 도입 검토 (GitHub Actions → 서버 pull/build/restart)`
