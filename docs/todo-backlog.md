...
- selector 보조 카피에는 현재 지원 대상이 로토무 appliance 폼과 일부 지역 폼에 한정된다는 점, 그리고 `가라르 프리져/썬더/파이어`, `팔데아 켄타로스`는 별도 backlog로 유지된다는 점을 명시합니다.
- `21-3-6-5-2` 구현 메모:
- 전설/환상 폼 1차 지원은 현재 `formKey` 일반 폼 모델에 안전하게 얹을 수 있는 `기라티나 -> 오리진폼`, `쉐이미 -> 스카이폼`까지만 엽니다.
- 두 종은 snapshot에 이미 대응 폼 데이터가 있고, 타입/특성/스탯/아트워크 전환을 selector 하나로 처리할 수 있으며 별도 move override가 없어도 현재 national-dex learnset 모델과 큰 correctness 충돌이 없습니다.
- `버드렉스 rider` 폼은 현재 snapshot에는 존재하지만 `블리자드랜스`, `아스트랄비트` 같은 rider signature move 노출과 복합 폼 의미를 함께 검토해야 하므로 이번 단계에서는 열지 않습니다.
- 후속 후보는 `버드렉스 rider`, `랜드로스 화신/영물`, `토네로스/볼트로스 영물`, `후파`, `멜메탈/마기아나 특수 폼`처럼 단순 selector 지원 여부를 다시 검토해야 하는 전설/환상 폼들입니다.
- `21-3-6-5-3` 구현 메모:
- 다수 폼/복합 규칙 후보 중 1차 구현은 `팔데아 켄타로스 breed`까지만 엽니다.
- `팔데아 켄타로스`는 세 breed가 모두 같은 national dex 아래에서 `formKey` 하나로 식별되고, 타입/아트워크/스탯/특성 전환이 selector만으로 자연스럽게 설명되며, 현재 national-dex move 모델에서도 `격분의뿔` 노출이 막히지 않아 최소 구현 범위에 맞습니다.
- `지가르데`는 `10%`, `50%`, `퍼펙트`, `스웜체인지`가 섞여 있어 단순 selector로 열 경우 상태 전환 의미와 폼 경계가 불명확해집니다. 현재 snapshot에는 `mega` 성격의 불안정한 폼 데이터까지 섞여 있어 이번 단계에서는 보류합니다.
- 후속 검토 대상은 `지가르데`, `버드렉스 rider`, `랜드로스 화신/영물`, `토네로스/볼트로스 영물`, `후파`, `마기아나 특수 폼`처럼 selector 하나 이상 또는 추가 move/상태 규칙 판단이 필요한 케이스들입니다.
