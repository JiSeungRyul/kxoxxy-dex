...
export function RandomTeamPage({ pokemonOptions }: RandomTeamPageProps) {
  ...
  async function rollRandomTeam() {
    ...
    try {
      const response = await fetch(`/api/pokedex/catalog?view=teams&dexNumbers=${sampledDexNumbers.join(",")}`);
  ...
  return (
    <section className="space-y-6">
      ...
      {error ? (
        <section className="rounded-[2rem] border border-ember/30 bg-ember/10 px-6 py-4 text-sm text-ember shadow-card sm:px-8">
          {error}
        </section>
      ) : null}

      {team.length === 0 ? (
        <section className="rounded-[2rem] border border-dashed border-border bg-card px-6 py-12 text-center shadow-card sm:px-8">
        ...
        <section className="rounded-[2rem] border border-border bg-card px-6 py-6 shadow-card sm:px-8">
          <div className="flex items-center justify-between gap-4">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.22em] text-muted-foreground">Result</p>
              <h3 className="mt-2 font-display text-3xl font-semibold tracking-[-0.05em] text-foreground">이번 랜덤 팀</h3>
            </div>
          </div>
          ...
          <div className="mt-6 grid gap-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6">
            {team.map((entry) => (
              <article
                ...
                className="rounded-[1.25rem] border border-border bg-background px-2.5 py-3 text-center shadow-sm"
              >
